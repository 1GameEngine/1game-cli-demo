...
```json
{
  "scripts": {
    ...
    "gameplay:replay:single": "1gameplay bundle-player-html out/debug.1gamerecord --out out/replay-single.html --single-html"
  }
}
```
...
Keyboard `detail.code` values follow the Web `KeyboardEvent.code` standard (there is no 1Game-specific key enum).
...
Inject pointer or keyboard events (headless contract):

```bash
# 1) Macro click (CLI expands to touch down + touch up)
pnpm exec 1gameplay step out/debug.1gamerecord --ms 16 --event '{"type":"click","sceneId":"main","data":{"x":272,"y":148}}'

# 2) Macro keypress (CLI expands to keyboard press + release)
pnpm exec 1gameplay step out/debug.1gamerecord --ms 16 --event '{"type":"keypress","sceneId":"main","data":{"code":"ArrowLeft"}}'

# 3) Advanced raw keyboard events (full pressed-key snapshots)
pnpm exec 1gameplay step out/debug.1gamerecord --ms 16 --event '{"type":"keyboard","sceneId":"main","data":[{"code":"ArrowLeft","key":"ArrowLeft","time":1000,"shift":false,"meta":false,"alt":false,"ctrl":false}]}'
pnpm exec 1gameplay step out/debug.1gamerecord --ms 16 --event '{"type":"keyboard","sceneId":"main","data":[]}'
```

Notes:

- Runtime worker event `type` values are `touch`, `hover`, `keyboard`, and `visibleChange`.
- CLI also supports macro events `click` and `keypress`, which are expanded to runtime events before dispatch.
- `keydown` and `keyup` are still DOM-side names; use `keypress` macro or raw `keyboard` snapshots instead.
...
Export a single-file replay HTML:
