...
- `1gameplay create --entry <path> --out <file.1gamerecord>`. Prefer paths under `out/` (for example `out/debug.1gamerecord`) so archives stay next to `1game build` output and match the default `.gitignore` from `1game init`.
...
For deterministic debugging, pass `--ms` explicitly in every `1gameplay step` command (for example `--ms 16` for real-time loops, or a game-specific fixed tick interval).
...
- `keydown` → add key to scene press-set and send `keyboard` snapshot
- `keyup` → remove key from scene press-set and send `keyboard` snapshot

`keydown` and `keyup` are **1gameplay CLI macros** (schema v5 archives). They expand to `keyboard` events using the engine keyboard snapshot.

`1gameplay step` can omit `--ms`; default step interval is `100` ms.

For low-level control without macros, send runtime `keyboard` events directly:

- `keyboard` with a non-empty `data` array for the full pressed-key snapshot
- `keyboard` with `data: []` to release all pressed keys in that scene
...
- `frames list` success schema is `1gameplay.frames.list.v1` (`result.rows` holds summary rows).
- Non-frame command failures default to stdout JSON with schema `1gameplay.error`.
