...
- `1gameplay bundle-player-html <file.1gamerecord> --out <replay.html> [--title <s>]` (prefer `--out out/replay.html` so the HTML is ignored alongside other `out/` artifacts; deploy `replay.html`, `player.bundle.js`, and the copied `.1gamerecord` together)
- `1gameplay frames delete <file.1gamerecord> --ranges <spec> [--compact]`

Archives are regular SQLite-backed `.1gamerecord` files, not directories. `1gameplay step --note` has been removed; use `comments add` for frame annotations.
