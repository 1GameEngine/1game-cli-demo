...
function applyMove(draft: GameState, direction: Direction): void {
  ...
  if (draft.score >= 2048 && draft.phase === 'playing') draft.phase = 'won';
}
...
function Game() {
  ...
  return (
    ...
    >
      ...
      />
      <text x={SCENE_WIDTH - 168} y={44} width={72} height={16} text="分数" textAlign="center" textColor="#eee4da" textSize="12" />
      <text
        x={SCENE_WIDTH - 168}
        y={60}
        width={72}
      ...
      <node
        ...
        backgroundColor="#bbada0"
      />
      <text x={SCENE_WIDTH - 88} y={44} width={72} height={16} text="最高" textAlign="center" textColor="#eee4da" textSize="12" />
      <text
        x={SCENE_WIDTH - 88}
        y={60}
        width={72}
      ...
      {store.displayTiles
        ...
        .map((tile) => {
          ...
          return (
            <group key={tile.id}>
              <node x={x} y={y} width={size} height={size} shape="roundedRect(6 6 6 6)" backgroundColor={style.bg} />
              <text
                x={x}
      ...
      {store.phase === 'lost' ? (
        <group x={GRID_X} y={GRID_Y} width={GRID_WIDTH} height={GRID_HEIGHT} clickable onClick={restartGame}>
          <node x={0} y={0} width={GRID_WIDTH} height={GRID_HEIGHT} shape="roundedRect(10 10 10 10)" backgroundColor="#00000088" />
          <text
            x={0}
            y={GRID_HEIGHT / 2 - 40}
            width={GRID_WIDTH}
