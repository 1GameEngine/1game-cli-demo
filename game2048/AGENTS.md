...
- 尚未激活时，可直接读取 `node_modules/@1game/skill/skills/1game-game-dev/SKILL.md` 作为兜底；若已安装 `@1game/skill`，运行 `pnpm exec @1game/skill activate --cursor` 即可完成激活（按需加 `--claude`/`--agents`/`--global`），并提醒用户后续补齐 IDE 激活。
