...
- `pnpm exec 1game buildjs --entry src/game.tsx --outDir out`（可选：仅用于定位 worker 构建问题）
...
1. 先构建浏览器产物：
   ...
   - 如需单独排查 worker 构建，可额外执行 `pnpm exec 1game buildjs --entry src/game.tsx --outDir out`
2. 创建可回放记录：
