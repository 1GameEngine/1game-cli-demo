...
4. 查询状态与渲染结果：
   - `pnpm exec 1gameplay frame query out/debug.1gamerecord --at last --select render --payload summary`（先获取真实 `sceneStableUid`）
   - `pnpm exec 1gameplay frame query out/debug.1gamerecord --at last --select runtime --select store:state --select render:sceneStableUid=<sceneStableUid> --payload summary`
5. 对比帧差异定位回归：
