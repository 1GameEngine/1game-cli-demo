...
function setInput(key: 'left' | 'right', pressed: boolean): void {
  commitChange(`input:${key}:${pressed ? 'down' : 'up'}`, (draft: GameState) => {
...
function Game() {
  ...
  return (
    ...
    >
      <text x={8} y={8} width={180} height={20} text={`Score: ${store.score}`} textColor="#f9fafb" textSize="16" />
      <text x={190} y={8} width={120} height={20} text={store.phase.toUpperCase()} textColor="#facc15" textAlign="right" textSize="16" />
      ...
      >
        ...
        <text x={0} y={7} width={64} height={18} text="Left" textColor="#fff" textAlign="center" textSize="16" />
      </group>
      ...
      >
        <node x={0} y={0} width={64} height={32} shape="roundedRect(8 8 8 8)" backgroundColor="#374151" />
        <text x={0} y={7} width={64} height={18} text="Right" textColor="#fff" textAlign="center" textSize="16" />
      </group>
    </scene>
  );
}
